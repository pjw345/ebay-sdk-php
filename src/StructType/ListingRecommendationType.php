...
class ListingRecommendationType extends AbstractStructBase
{
     ...
     * Set Type value
     * @param string $type
     * @return \StructType\ListingRecommendationType
     */
    ...
    public static function validateMetadataForArrayConstraintsFromSetMetadata(array $values = array())
    {
        ...
        foreach ($values as $listingRecommendationTypeMetadataItem) {
            // validation for constraint: itemType
            if (!$listingRecommendationTypeMetadataItem instanceof \StructType\MetadataType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Metadata property can only contain items of type \StructType\MetadataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\MetadataType $item
     * @return \StructType\ListingRecommendationType
     */
    public function addToMetadata(\StructType\MetadataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\MetadataType) {
            throw new \InvalidArgumentException(sprintf('The Metadata property can only contain items of type \StructType\MetadataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->Metadata[] = $item;
        return $this;
    }
