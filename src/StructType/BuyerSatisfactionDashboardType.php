...
class BuyerSatisfactionDashboardType extends AbstractStructBase
{
    ...
    /**
     * Set Status value
     * @uses \EnumType\BuyerSatisfactionStatusCodeType::valueIsValid()
     * @uses \EnumType\BuyerSatisfactionStatusCodeType::getValidValues()
     * @throws \InvalidArgumentException
     * @param string $status
     * @return \StructType\BuyerSatisfactionDashboardType
     */
    public function setStatus($status = null)
    {
        // validation for constraint: enumeration
        if (!\EnumType\BuyerSatisfactionStatusCodeType::valueIsValid($status)) {
            throw new \InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\BuyerSatisfactionStatusCodeType', is_array($status) ? implode(', ', $status) : var_export($status, true), implode(', ', \EnumType\BuyerSatisfactionStatusCodeType::getValidValues())), __LINE__);
        }
        $this->Status = $status;
        return $this;
    }
    ...
    public static function validateAlertForArrayConstraintsFromSetAlert(array $values = array())
    {
        ...
        foreach ($values as $buyerSatisfactionDashboardTypeAlertItem) {
            ...
            if (!$buyerSatisfactionDashboardTypeAlertItem instanceof \StructType\SellerDashboardAlertType) {
