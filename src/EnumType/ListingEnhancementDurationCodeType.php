 ...
 * - documentation: <span class="tablenote"><b>Note:</b> This enumerated type is no longer applicable, as the Featured First listing feature is being deprecated for all sites. If a seller attempts to add Featured First as a listing enhancement
 * (including <b>ListingEnhancement</b> field in an <b>AddItem</b> call and setting its value to <code>Featured</code>), it is allowed, but a warning will be returned announcing the deprecation of the feature. Starting August 15, 2019, this feature
 * cannot be added when creating a new listing or revising an existing listing. For existing listings that already have the feature, the seller will not be able to change the duration to <code>Lifetime</code>. On September 15, 2019, this feature will be
 * wired off. If a seller revises an existing listing that had this feature set, the <b>GalleryType</b> value will automatically default to <code>Gallery</code>. </span> <br> This enumerated type contains the list of values that can be used by the
 * seller to set the duration of a Featured Gallery in a fixed-price listing. Once set for a listing, the Featured Gallery duration can be increased from 'Days_7' to 'Lifetime' (throughout life of listing), but the duration cannot be decreased from
 * 'Lifetime' to 'Days_7'.
